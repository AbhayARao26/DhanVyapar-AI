...
def test_nbfc_recommendation():
    """Test NBFC Recommendation Agent"""
    print_header("TESTING NBFC RECOMMENDATION AGENT")
    ...
    for region in test_regions:
        ...
        # Get recommendations
        recommendations = agent.recommend_nbfcs_by_region(region, max_results=5)
        
        if recommendations["success"]:
            print(f"✅ Found {recommendations['total_found']} NBFCs in {region}")
            print("Top recommendations:")
            for i, nbfc in enumerate(recommendations["recommendations"][:3], 1):
                print(f"  {i}. {nbfc['name']}")
                print(f"     Type: {nbfc['classification_description']}")
                if nbfc.get('email'):
                    print(f"     Email: {nbfc['email']}")
        else:
            print(f"❌ Error: {recommendations['error']}")
            if recommendations.get('suggested_regions'):
                print(f"Suggested regions: {', '.join(recommendations['suggested_regions'])}")
    
    # Test with classification filter
    print(f"\n--- Testing with MFI Filter ---")
    mfi_recommendations = agent.recommend_nbfcs_by_region("Karnataka", classification_filter="MFI", max_results=3)
    if mfi_recommendations["success"]:
        print(f"✅ Found {mfi_recommendations['total_found']} MFI NBFCs in Karnataka")
        for nbfc in mfi_recommendations["recommendations"]:
            print(f"  - {nbfc['name']}")
    
    # Test region statistics
    print(f"\n--- Regional Statistics ---")
    stats = agent.get_regional_statistics("Maharashtra")
    if stats["success"]:
        print(f"Maharashtra Statistics:")
        print(f"  Total NBFCs: {stats['statistics']['total_nbfcs']}")
        print(f"  By Classification: {stats['statistics']['by_classification']}")
        print(f"  Deposit Accepting: {stats['statistics']['deposit_accepting']}")
...
def main():
    ...
    print("7. NBFC Recommendation Agent")
