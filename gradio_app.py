...
def get_nbfc_recommendations(region: str, classification_filter: str = "", language: str = "English") -> str:
    """Get NBFC recommendations based on user's region"""
    global current_user_id
    
    if not current_user_id:
        return "❌ No user selected. Please create or select a user first."
    
    user_data = user_database[current_user_id]
    
    if not region:
        return "❌ Please enter a region to search for NBFCs."
    
    try:
        # Get recommendations
        recommendations = nbfc_agent.recommend_nbfcs_by_region(
            region, 
            classification_filter=classification_filter if classification_filter else None,
            max_results=10
        )
        
        # Generate summary in user's preferred language
        user_lang = translation_agent.get_user_preferred_language(user_data)
        if language.lower() != user_lang:
            user_lang = language.lower()
        
        summary = nbfc_agent.generate_recommendation_summary(recommendations, user_lang)
        
        # Add additional details if successful
        if recommendations.get("success") and recommendations.get("recommendations"):
            summary += "\n\n## 📊 Regional Statistics\n"
            stats = nbfc_agent.get_regional_statistics(region)
            if stats.get("success"):
                summary += f"- Total NBFCs: {stats['statistics']['total_nbfcs']}\n"
                summary += f"- Deposit Accepting: {stats['statistics']['deposit_accepting']}\n"
                summary += f"- Non-Deposit Accepting: {stats['statistics']['non_deposit_accepting']}\n"
                
                if stats['statistics']['by_classification']:
                    summary += "\n**By Classification:**\n"
                    for classification, count in stats['statistics']['by_classification'].items():
                        summary += f"- {classification}: {count}\n"
        
        return summary
        
    except Exception as e:
        return f"❌ Error getting NBFC recommendations: {e}"
...
with gr.Blocks(title="Microfinance Agents System", theme=gr.themes.Soft()) as app:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("🏛️ NBFC Recommendations"):
            gr.Markdown("### Find NBFCs Near You")
            
            with gr.Row():
                region_input = gr.Textbox(label="Your Region/City", placeholder="e.g., Mumbai, Bangalore, Delhi, Karnataka")
                classification_filter = gr.Dropdown(
                    choices=["", "ICC", "MFI", "CIC", "IFC", "HFC"],
                    label="NBFC Type (Optional)",
                    value=""
                )
            
            with gr.Row():
                nbfc_language = gr.Dropdown(choices=["English", "Hindi", "Kannada"], label="Language", value="English")
                nbfc_btn = gr.Button("Find NBFCs", variant="primary")
            
            nbfc_result = gr.Markdown()
    ...
    nbfc_btn.click(
        fn=get_nbfc_recommendations,
        inputs=[region_input, classification_filter, nbfc_language],
        outputs=[nbfc_result]
    )
